...
if [ "${ACME_REGISTER_ACCOUNT}" == "1" ] && [ "${ACME_ISSUE}" == "1" ] && [ "${ACME_INSTALL_CERT}" == "1" ]; then
    if [ -f "${CERT_RENEW_DT_FILE}" ]; then
        log "Let's wait cron job: skip processing, file exists: ${CERT_RENEW_DT_FILE}"
        exit 0
    fi
fi
...
find "${SITES_DIR}" -type f -name "*.conf" | sort | while read -r conf_file; do
    log "Work with conf file: ${conf_file}" 1 0
    ...
    read -ra domains <<< "$(
        grep -E '^[[:space:]]*#([[:space:]]*)acme' "${conf_file}" |
        ...
        if [ "${ACME_INSTALL_CERT}" == "1" ]; then
            ...
            eval "set -x ; \
                      eval acme.sh ${ACME_OPTS} \
                          ...
                          --reloadcmd      \'RELOADCMD=1 /acme_handler.sh\'; \
                  { set +x ; } 2>/dev/null"
        fi
    else
        log " [skip]" 0
